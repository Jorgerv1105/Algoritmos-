...
typedef struct {
    ...
    int prioridad;        // Prioridad del proceso (a menor valor, mayor prioridad)
...
int encontrar_proceso_con_mayor_prioridad(Proceso *procesos, int n, int tiempo_actual) {
    int index = -1;
    int prioridad_maxima = 10000; 

    for (int i = 0; i < n; i++) {
        if (procesos[i].tiempo_llegada <= tiempo_actual && !procesos[i].completado) {
            if (procesos[i].prioridad < prioridad_maxima) {
                prioridad_maxima = procesos[i].prioridad;
                index = i;
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        printf("Ingrese la prioridad del proceso %c (menor valor = mayor prioridad): ", procesos[i].nombre);
    ...
    while (procesos_completados < n) {
        int index = encontrar_proceso_con_mayor_prioridad(procesos, n, tiempo_actual);
        if (index != -1) {
    ...
    printf("\n%10s%15s%15s%15s%15s%20s%20s\n", "Proceso", "Llegada", "Ráfaga CPU", "Prioridad", "Inicio", "Tiempo Espera", "Tiempo Retorno");
    for (int i = 0; i < n; i++) {
        printf("%10c%15d%15d%15d%15d%20d%20d\n",
            procesos[i].nombre,
